...
class ZLAC8015D:
	...
	def __init__(self, port=None):
		...
		# 	print(f"Failed to connect to Modbus device at port {self._port}")
		# else:
		# 	print(f"Connected to Modbus device at port {self._port}")
		self.ID = 1
	...
	def modbus_fail_read_handler(self, ADDR, WORD):
		...
		while not read_success:
			...
			# if result.isError():
			# 	# Handle the error by raising an exception or logging it
			# 	raise Exception(f"Failed to read registers at address {ADDR} with count {WORD}: {result}")
			
			# If no error, return the registers data
	...
	def get_wheels_tick(self):
		...
		# l_tick = np.int64(((l_pul_hi & 0xFFFF) << 16) | (l_pul_lo & 0xFFFF))

		l_tick = np.int64(((l_pul_hi & 0xFFFF) << 16) | (l_pul_lo & 0xFFFF))
		r_tick = np.int64(((r_pul_hi & 0xFFFF) << 16) | (r_pul_lo & 0xFFFF))
	...
	def SetSerialport(self,port):
		...
		if self.client == None:
			self._port = port
			# print(f"select port name {port}")
			self.client = ModbusClient(method='rtu', port=self._port, baudrate=115200, timeout=1)
			if (self.client.connect()):
				result = self.client.read_input_registers(0x3100, 20, unit=1)
				print(f"result = {result}")
				print(f"Motor Serialport {self._port} Opened")
				while(1):
					try:
						readreg = self.client.read_holding_registers(0, 15, unit=1)
	  
						if readreg.isError():
							print(f"Modbus Error: {readreg}")
						else:
							for register in readreg.registers:
								print(register)
			...
			print("Motor Serialport is already connected")
			self.client.close()
			self._port = port
			self.client = ModbusClient(method='rtu', port=self._port, baudrate=115200, timeout=1)
			self.client.connect()
			
	def SerialportClosed(self):
		if self.client != None:
			self.client.close()
			self._port = None
			print("Motor SerialportClosed")
